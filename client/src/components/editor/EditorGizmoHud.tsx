...
import { EDITOR_MATERIAL_FAMILIES, type EditorGizmoMode, type EditorMaterialFamilyId } from '@/lib/editorTools';
...
export default function EditorGizmoHud({
  ...
  return (
    <div
      className="absolute top-3 left-3 z-20 w-64 pointer-events-auto text-[#e8dfd0] text-[11px]"
    ...
    >
      <div className="rounded border border-amber-800/50 bg-black/75 backdrop-blur-sm p-2 space-y-2">
        ...
        <div className="text-[10px] text-slate-400 truncate">
          {selectedName ? `Selected · ${selectedName}` : 'Click a mesh to select'}
        </div>
        {selectedName && (
          <>
            <div className="text-[10px] uppercase tracking-wide text-amber-400/80">Material</div>
              ...
              {EDITOR_MATERIAL_FAMILIES.map((f) => (
                <button
                  ...
                  className={`px-1.5 py-0.5 rounded border text-[10px] ${
                    familyId === f.id ? 'border-amber-400 bg-amber-900/40' : 'border-white/15'
                  }`}
        ...
        <p className="text-[9px] text-slate-500 leading-snug">
          W move · E rotate · T scale · click mesh · cloth / leather / metal / magic / water / brick / wood
        </p>
      </div>
    </div>
  );
