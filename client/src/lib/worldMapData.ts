...
export function getNearestIsland(position: THREE.Vector3): WorldIslandData | null {
  let nearest: WorldIslandData | null = null;
  let minDist = Infinity;
  
  for (const island of WORLD_ISLANDS) {
    const dist = Math.sqrt(
      Math.pow(position.x - island.position.x, 2) + 
      Math.pow(position.z - island.position.z, 2)
    );
    if (dist < minDist) {
      minDist = dist;
      nearest = island;
...
export function getEnemyShipsInArea(center: THREE.Vector3, radius: number): EnemyShipData[] {
  return WORLD_ENEMY_SHIPS.filter(ship => {
    const dist = Math.sqrt(
      Math.pow(center.x - ship.patrolCenter.x, 2) + 
      Math.pow(center.z - ship.patrolCenter.z, 2)
    );
    return dist < radius + ship.patrolRadius;
  });
}
